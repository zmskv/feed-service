...
Без Docker и без базы — с `STORAGE=memory` (дефолт в `example.env`) все данные живут в оперативной памяти процесса и пропадают при перезапуске, зато поднимается мгновенно. `.env` в корне подхватывается сам через [godotenv](https://github.com/joho/godotenv) — если файла нет, просто использует настоящие переменные окружения/дефолты, ошибки не будет.

Или через [Taskfile](https://taskfile.dev) (`task run`, `task build`, `task test`). Для postgres нужна поднятая БД: `docker-compose up postgres migrate` (поднимет только базу и накатит миграции, без запуска самого приложения в контейнере), затем `STORAGE=postgres go run ./cmd/feed-service`.

## Конфигурация

Только переменные окружения (никаких CLI-флагов), см. [example.env](example.env).

| Env | По умолчанию | Описание |
|---|---|---|
| `STORAGE` | `memory` | `memory` \| `postgres` |
| `ADDR` | `:8080` | адрес HTTP |
| `DSN` | — | полная строка подключения; если задана — переопределяет `PG*` |
| `PGHOST` / `PGPORT` | `localhost` / `5432` | адрес Postgres |
| `PGUSER` / `PGPASSWORD` | `feed` / `feed` | учётные данные |
| `PGDATABASE` | `feed` | имя базы |
| `PGSSLMODE` | `disable` | sslmode |

`DSN` нужен редко — покрывает случаи, которые не выразить через отдельные `PG*` (нестандартные параметры подключения, unix-сокет и т.п.). В `docker-compose.yml` `.env` из корня проекта подхватывается автоматически (стандартное поведение Compose), DSN контейнера `app` собирается из тех же `PG*`-переменных, что и в `example.env`, но с хостом `postgres` (имя сервиса в сети контейнеров, не `localhost`).
...
Курсор дополнительно привязан к конкретному connection, который его выдал (список постов / комментарии именно этого поста / ответы именно на этот комментарий) — курсор, полученный из комментариев поста A, не сработает для поста B или для списка постов, а вернёт явную ошибку вместо тихо неправильной страницы.
...
SELECT * FROM ranked WHERE rn <= $2 ORDER BY post_id, rn
```

Внешний `ORDER BY` обязателен: `ROW_NUMBER()` детерминированно считает `rn`, но без сортировки самого `SELECT` Postgres ничего не гарантирует про порядок выдачи строк — без него код, который потом режет `rn <= first`, мог бы получить страницу не по рангу, а как повезёт с физическим порядком.
...
{ErrInvalidID,                     "INVALID_ID"}, // не пускает наружу сырой текст uuid.Parse
```

Всё, что не распознано, уходит с общим текстом `"internal server error"` (код `INTERNAL_ERROR`) — реальная ошибка при этом логируется на сервере, наружу не течёт. Синтаксис/валидацию самого GraphQL (неизвестное поле, не тот тип аргумента) это не трогает — те ошибки и так безопасны и полезны клиенту, редактируются только ошибки, вернувшиеся из наших резолверов. Пост по несуществующему id — не ошибка вообще, а пустой ответ (`null`): схема объявляет `Post` без `!`, отсутствие — валидный результат, не сбой.
...
- `infrastructure/postgres` — только тесты на реальной базе (SQL руками не гоняется), которые сами себя пропускают, если не задана `TEST_DATABASE_URL`.
...
```bash
task test               # без -race (нужен cgo, на Linux/CI/Docker работает)
